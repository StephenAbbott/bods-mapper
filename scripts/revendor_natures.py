#!/usr/bin/env python3
"""Re-vendor the Companies House PSC nature/super-secure descriptions.

Regenerates ``src/bods_mapper/natures_data.py`` from the upstream
``psc_descriptions.yml`` so the vendored copy is refreshed *reproducibly*
instead of hand-maintained — guarding against drift when Companies House adds
or revises codes. (bods-stream maps the live PSC stream, so the company-level
``statement_description`` block is not vendored here; see OpenCheck for that.)

We vendor two of the four upstream blocks:
  * ``short_description``        → ``PSC_NATURE_DESCRIPTIONS``   (86 nature codes)
  * ``super_secure_description`` → ``SUPER_SECURE_DESCRIPTIONS`` (2 codes)

Usage:
    python scripts/revendor_natures.py              # download + rewrite
    python scripts/revendor_natures.py --source <path-or-url>
    python scripts/revendor_natures.py --check      # CI: exit 1 if stale
...
TARGET = Path(__file__).resolve().parents[1] / "src" / "bods_mapper" / "natures_data.py"
...
_HEADER = '''\
"""Companies House PSC nature/super-secure descriptions → human-readable text.
...
Do NOT edit by hand. Regenerate with::
    ...
    python scripts/revendor_natures.py
...
def main() -> int:
    ...
    if args.check:
        ...
        if current != generated:
            print(f"STALE: {TARGET} differs from upstream. Run revendor_natures.py.")
            return 1
